...
int FloatIntStrTestErrorHandler(RIB_HANDLE rib, int code, int severity, PRIB_ERROR error);
...
#define _TABLE_LENGTH
    ...
    if ((PRIB_SUBDIVFUNC)RiProcDelayedReadArchive != (PRIB_SUBDIVFUNC)(((PRIB_INSTANCE)rib)->ritable[kRIB_PROCDELAYEDREADARCHIVE])) {
    ...
    if (!pf || pf[0] != (float)1.234 || pf[1] != (float)2.345 || pf[2] != (float)3.456) {
        if (pf)
