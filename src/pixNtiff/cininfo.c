...
int cininfo(char *filename, int multiplefiles) {
    ...
    /* Read file header. */
    if (fread((char *)&header, 1, sizeof(CINEON_FILEHEADER), cin) < sizeof(CINEON_FILEHEADER))
        goto Error;
    ...
    if (fread((char *)&imgdesc, 1, sizeof(CINEON_IMAGEDESC), cin) < sizeof(CINEON_IMAGEDESC))
        goto Error;
