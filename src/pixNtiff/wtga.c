...
int WriteTga(PBITMAP pBmp, char *filename, int rle) {
    ...
    if (pBmp->sampleformat != BITMAP_UINT8 || pBmp->bitspersample != 8 || (pBmp->nsamples != 3 && pBmp->nsamples != 4)) {
        return 1;
    }
     ...
     */
    if (pBmp->orientation != BITMAP_TOPLEFT && pBmp->orientation != BITMAP_TOPRIGHT) {
    ...
    if (pBmp->orientation == BITMAP_TOPRIGHT || pBmp->orientation == BITMAP_BOTRIGHT) {
    ...
    /* Write TGA header. */
    if (fwrite((char *)&tgaheader, 1, sizeof(TGAHEADER), tga) < sizeof(TGAHEADER))
        goto Error;
        ...
        if (pixelsize == 32) {
            ...
            while (pp >= pBmp->pbits) {
                ...
                if (b[1] != p[2] || b[2] != p[1] || b[3] != p[0] || b[4] != p[3]) {
                    s = p;
                    while (p < last) {
                        if (((p + 4) == last) || ((p + 4) < last && (p[0] != p[3] || p[1] != p[4] || p[2] != p[5] || p[3] != p[6]))) {
                            p += 4;
                            count++;
                        }
                        else {
            ...
            while (pp >= pBmp->pbits) {
                ...
                if (b[1] != p[2] || b[2] != p[1] || b[3] != p[0]) {
                    s = p;
                    while (p < last) {
                        if (((p + 3) == last) || ((p + 3) < last && (p[0] != p[3] || p[1] != p[4] || p[2] != p[5]))) {
                            p += 3;
                            count++;
                        }
                        else {
