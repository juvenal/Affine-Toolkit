...
int WriteTiff(PBITMAP pBmp, char *filename) {
    ...
    if (pBmp->sampleformat == BITMAP_IEEE) {
        ...
        TIFFSetField(tif, TIFFTAG_PHOTOMETRIC, (pixelsize == 8 ? PHOTOMETRIC_MINISBLACK : PHOTOMETRIC_RGB));
    ...
    if (pBmp->nsamples == 4 && (pBmp->bitspersample == 8 || pBmp->bitspersample == 16)) {
        uint16 us[1];
...
int WriteTiff(PBITMAP pBmp, char *filename) {
    ...
    static IFD_ENTRY ifde[] = {
        /* array of IFD Entries to append to ifd */
        /* NOTE:  Things marked by "(set later)" can't be hardcoded here and
         *        are set during runtime.
         */
        /* NewSubFileType, TAG = 0xFE */
        0x00,
        0xfe, /* bytes  0-1  Tag ID                                    */
        0x00,
        LONG_TYPE, /* bytes  2-3  Field type                           */
        0x00,
        0x00, /* bytes  4-7  Count of how many of the field            */
        0x00,
        0x01, /*             (only one value SHORT or LONG             */
        0x00,
        0x00, /* bytes 8-11  The value (if it fits), otherwise offset. */
        0x00,
        0x00, /*             (set later)                               */
        /* ImageWidth, TAG = 0x100 */
        0x01,
        0x00, /* bytes  0-1  Tag ID                                    */
        0x00,
        0x00, /* bytes  2-3  Field type (set later)                    */
        0x00,
        0x00, /* bytes  4-7  Count of how many of the field            */
        0x00,
        0x01, /*             (only one value SHORT or LONG             */
        ...
        0x01,
        0x02, /* bytes  0-1  Tag ID                                    */
        0x00,
        SHORT_TYPE, /* bytes  2-3  Field type                           */
        0x00,
        0x00, /* bytes  4-7  Count of how many of the field            */
        0x00,
        0x03, /*  (three values)                                       */
        0x00,
        0x00, /* bytes 8-11  Use offset. (3 sets of 2 bytes won't fit  */
        0x00,
        0x00, /*             into 4 bytes.  Use an offset. (set later) */
        /* Compression, TAG = 0x103 */
        0x01,
        0x03, /* bytes  0-1  Tag ID                                    */
        0x00,
        SHORT_TYPE, /* bytes  2-3  Field type                          */
        0x00,
        0x00, /* bytes  4-7  Count of how many of the field            */
        ...
        0x01, /* (only 1 value)                                        */
        0x00,
        0x01, /* bytes 8-11  The value of 1.                           */
        0x00,
        0x00, /*   (1==no compression, left justified SHORT_TYPE)      */
              /*   Inshort put value in 2nd byte not the 4th byte.     */
        /* PhotometricInterpretation, TAG = 0x106 */
        0x01,
        0x06, /* bytes  0-1  Tag ID                                    */
        0x00,
        SHORT_TYPE, /* bytes  2-3  Field type                          */
        0x00,
        0x00, /* bytes  4-7  Count of how many of the field            */
        ...
        0x01, /* (only 1 value)                                        */
        0x00,
        0x02, /* bytes 8-11  The value                                 */
        0x00,
        0x00, /*   (1==no compression, left justified SHORT_TYPE)      */
              /*   Inshort put value in 2nd byte not the 4th byte.     */
        /* StripOffsets, TAG = 0x111 */
        0x01,
        0x11, /* bytes  0-1  Tag ID                                    */
        0x00,
        LONG_TYPE, /* bytes  2-3  Field type                           */
        0x00,
        0x00, /* bytes  4-7  Count of how many of the field            */
        ...
        0x01, /* (only 1 value)                                        */
        0x00,
        0x03, /* bytes 8-11  The value of 3.                           */
        0x00,
        0x00, /*   (3==3 color sample R, G and B.  Left justified      */
              /*   SHORT_TYPE.   Inshort put value in 2nd byte not the */
              /*   4th byte.                                           */
        /* RowsPerStrip, TAG = 0x116 */
        0x01,
        0x16, /* bytes  0-1  Tag ID                                    */
        0x00,
        LONG_TYPE, /* bytes  2-3  Field type                           */
        0x00,
        0x00, /* bytes  4-7  Count of how many of the field            */
        ...
        0x01, /*             (only one value)                          */
        0x00,
        0x00, /* bytes 8-11  Offset to value                           */
        0x00,
        0x00, /*             (set later)                               */
        /* XResolution, TAG = 0x11a */
        0x01,
        0x1a, /* bytes  0-1  Tag ID                                    */
        0x00,
        RATIONAL_TYPE, /* bytes  2-3  Field type                       */
        0x00,
        0x00, /* bytes  4-7  Count of how many of the field            */
        0x00,
        0x01, /*             (Only one value)                          */
        ...
        0x01, /* bytes 8-11  The value of 1.                           */
        0x00,
        0x00, /*   (1==compacked RGBRGBRGB style)                      */
        /* ResolutionUnit, TAG = 0x128 */
        0x01,
        0x28, /* bytes  0-1  Tag ID                                    */
        0x00,
        SHORT_TYPE, /* bytes  2-3  Field type                          */
        0x00,
        0x00, /* bytes  4-7  Count of how many of the field            */
        ...
        0x01, /* (only 1 value)                                        */
        0x00,
        0x02, /* bytes 8-11  The value of 3.                           */
        0x00,
        0x00, /*   (2==inch (default used).  Value is a left justified */
              /*   SHORT_TYPE.   Inshort put value in 2nd byte not the */
              /*   4th byte.  (Big-Endian)                             */
        /* Software, TAG = 0x131 */
        0x01,
        0x31, /* bytes  0-1  Tag ID                                    */
        0x00,
        ASCII_TYPE, /* bytes  2-3  Field type                          */
        0x00,
        0x00, /* bytes  4-7  Count of how many of the field            */
        0x00,
        sizeof(SoftwareValue), /* (length of string for count)         */
        0x00,
        0x00, /* bytes 8-11  Offset to string (set later).             */
        0x00,
        0x00, /*                                                       */
    };
    static char ifd[2] = {
        /* First part of IFD anyway. */
        0x00,
        sizeof(ifde) / sizeof(IFD_ENTRY), /* Number of IFD Entries ().   */
    };
    FILE *fp;
    int i;
    char *p;
