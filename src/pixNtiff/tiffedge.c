...
int Sobel8(PBITMAP original, PBITMAP target, float threshold) {
    ...
    if (!original || !target || (original->xres != target->xres) || (original->yres != target->yres) || (original->nsamples != 1) || (original->bitspersample != 8) || (target->nsamples != 1) || (target->bitspersample != 8)) {
        return 1; /* Error */
    }
...
int SobelIEEEFP(PBITMAP original, PBITMAP target, float threshold) {
    ...
    if (!original || !target || (original->xres != target->xres) || (original->yres != target->yres) || (original->sampleformat != BITMAP_IEEE) || (original->nsamples != 1) || (target->bitspersample != 32)) {
        return 1; /* Error */
    }
...
int Overlay(PBITMAP source, PBITMAP target) {
    ...
    if (!source || !target || (source->xres != target->xres) || (source->yres != target->yres) || (source->sampleformat != BITMAP_UINT8) || (target->sampleformat != BITMAP_UINT8)) {
        printf("ERROR:  overlay().\n");
        return 1;
    }
