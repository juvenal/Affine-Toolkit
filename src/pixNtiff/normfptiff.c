...
int NormalizeFPData(char *fin, char *fout, int ZeroMin, int ZeroOutInf, float Infinity, int Verbose, float Minimum, float Maximum);
...
int main(int argc, char **argv) {
    ...
    while (i < argc) {
        if (argv[i][0] == '-') {
            ...
            else if (!strcmp(&argv[i][1], "Minimum") || !strcmp(&argv[i][1], "min")) {
                i++;
            ...
            else if (!strcmp(&argv[i][1], "Maximum") || !strcmp(&argv[i][1], "max")) {
                i++;
