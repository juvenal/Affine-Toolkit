...
typedef int (*PRIB_ERRORFILTERPROC)(RIB_HANDLE rib, int code, int severity, PRIB_ERROR error);
...
#define /* RtBoolean */ RibShouldFreeData(/* RIB_HANDLE */ rib)        \
    ((rib) && (((PRIB_INSTANCE)(rib))->status & kRIB_STATUS_KEEP_DATA) \
...
#define /* size_t */ RibGetBufferLength(/* RIB_HANDLE */ rib) \
    (((rib) && ((PRIB_INSTANCE)(rib))->pbuf)                  \
         ? ((PRIB_INSTANCE)(rib))->pbuf->buflength            \
         : 0L)
...
typedef int (*PReadArrayFunc)(RIB_HANDLE hrib, RIB_UINT32 options, int numberof, void *p);
...
extern
    ...
    RibReadStringArray(RIB_HANDLE hrib, RIB_UINT32 options, int numberof, RtString **array),
    RibReadIntegerArray(RIB_HANDLE hrib, RIB_UINT32 options, int numberof, RtInt **array),
    RibReadFloatingPointArray(RIB_HANDLE rib, RIB_UINT32 options, int numberof, RtFloat **array),
    RibReadRIBCallParameters(RIB_HANDLE rib,
                             char *cparams, /* Refer to ribhash/cparams.c. */
                             int asizes[],
                             void *params),
    RibReadArrayAndLength(RIB_HANDLE hrib, RIB_UINT32 options, int *numberof, void **pp),
...
extern
    ...
    RibGetVectorCache(RIB_HANDLE rib,
                      ...
                      RtToken **tokens,
                      RtPointer **params),
    RibGetVectorCacheInfo(RIB_HANDLE rib, RtInt *n, RtInt *nvertices, unsigned int **types, unsigned int **sizes);
...
#endif
    int
    RibReadParameterList(RIB_HANDLE rib, int require, int uniform, int varying, int vertex, RtInt *pn, RtToken **paTokens, RtPointer **paParams),
    RibGetParameterTypeInfo(RIB_HANDLE rib,
                            int *uniform,
                            int *varying,
                            int *vertex),
    RibErrorNumberOfValues(RIB_HANDLE hrib, RtToken parameterID, RtInt expected_nitems, RtInt received_nitems);
...
extern
    ...
    RibDefaultErrorHandler(RIB_HANDLE hrib, int code, int severity, PRIB_ERROR error);
...
#endif
    RtVoid
    RibNuCurvesV(RtInt ncurves, RtInt nvertices[], RtInt order[], RtFloat knot[], RtFloat min[], RtFloat max[], RtInt n, RtToken tokens[], RtPointer parms[]),
    RibImplicitV(RtInt a, RtInt b[], RtInt c, RtFloat d[], RtInt e, RtFloat f[], RtInt g, RtFloat h[], RtInt n, RtToken tokens[], RtPointer parms[]),
