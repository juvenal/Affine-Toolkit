...
int RibHandlePrefix(RIB_HANDLE rib) {
    ...
    while (RibIsanEncodedDefinition(c) || RibIsaWhiteSpace(c) || archive_record) {
        if (archive_record) {
 ...
 *
 */
int RibReadStringArray(RIB_HANDLE hrib, unsigned int options, int numberof, RtString **pp) {
    auto PRIB_INSTANCE rib = (PRIB_INSTANCE)hrib;
    RtBoolean brackets;
    int rc = 0;
    auto int c;
    ...
    if ('[' == c) {
         ...
         */
        if (kRIB_BRACKETS_REQUIRED == utmp || (kRIB_BRACKETS_PARAML == utmp && numberof > 1)) {
 ...
 */
int RibReadFloatingPointArray(RIB_HANDLE hrib, RIB_UINT32 options, int numberof, float **pp) {
    auto PRIB_INSTANCE rib = (PRIB_INSTANCE)hrib;
    int tmp;
    RtBoolean brackets;
    int rc = 0;
...
int RibReadIntegerArray(RIB_HANDLE hrib, unsigned int options, int numberof, RtInt **pp) {
    auto PRIB_INSTANCE rib = (PRIB_INSTANCE)hrib;
    RtBoolean brackets;
    int rc = 0;
    int unget = -1; /* Don't unget a char before error msg. */
    ...
    if ('[' == c) {
         ...
         *    an error by looking at the kRIB_BRACKETS_* flag bits.
         */
        if (kRIB_BRACKETS_REQUIRED == i || (kRIB_BRACKETS_PARAML == i && numberof > 1)) {
            rib->error.type = kRIB_ERRTYPE_OPEN_BRACKET;
            RibSetError(rib, RIE_SYNTAX, RIE_ERROR, &(rib->error));
...
int RibReadArrayAndLength(RIB_HANDLE hrib, RIB_UINT32 options, int *pnumberof, void **pp) {
    auto PRIB_INSTANCE rib = (PRIB_INSTANCE)hrib;
    ...
    if ((brackets && (kRIB_BRACKETS_NONE == (options & kRIB_BRACKETS_MASK))) || (!brackets && (kRIB_BRACKETS_REQUIRED == (options & kRIB_BRACKETS_MASK)))) {
        rib->error.type = kRIB_ERRTYPE_OPEN_BRACKET;
        RibSetError(rib, RIE_SYNTAX, RIE_ERROR, &(rib->error));
    ...
    while (*c != kRIB_CPARAMS_NULL && *c != kRIB_CPARAMS_VECTORFORMAT) {
        switch (*c) {
            ...
            case kRIB_CPARAMS_BASIS:
                ...
                if (EOF == rc)
                    goto Error;
                if (rc == '\"' || rc == 0317 || rc == 0320 || ((rc >= 0220) && (rc <= 0243))) {
...
int RibReadFileForToken(RIB_HANDLE rib, char *table) {
    ...
    if (rib && table) {
        ...
        for (;;) {
            ...
            for (i = 0; i < l; i++) {
                if (c != *t) {
                    if (!*t && (RibIsaBinaryChar(c) || RibIsaWhiteSpace(c) || RibIsaDelimitor(c))) {
                        t++;
                        RibUngetChar(rib, c);
                        return *t;
                    }
