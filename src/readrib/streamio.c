 ...
 *
 */
static PRIB_INSTANCE RibOpenWith(char *filename, PRIB_INSTANCE prib, PRIB_INSTANCE parent, PRIB_HASHTABLE hashtable);
...
unsigned int RibFindFile(PRIB_INSTANCE prib, char *filename) {
     ...
     *    terminating '\0'.
     */
    if (prib->archivelnamebuffer == 0 || prib->archivelnamebuffer - prib->archivelmaxpath < lfilename + 1) {
        if (prib->archivenamebuffer) {
            _RibFree(prib->archivenamebuffer);
        }
        prib->archivelnamebuffer = prib->archivelmaxpath + lfilename + kRIB_MAXFILELENGTH;
    ...
    do {
        ...
        *b++ = '/';
        ...
        if (!stat(prib->archivenamebuffer, &statbuf) && !(statbuf.st_mode & S_IFDIR))
...
static PRIB_INSTANCE RibOpenWith(char *filename,
    ...
    if (prib) {
        ...
        rib->status |= (prib->status & (kRIB_STATUS_FRAMEBLOCK | kRIB_STATUS_WORLDBLOCK));
...
RIB_HANDLE RibOpen(char *filename, int rilevel, PRIB_RITABLE table) {
     ...
     *    return an error.
     */
    if (table && (rilevel < kRIB_LAST_RI || rilevel >= 0x3C100000)) {
...
int RibBegin(RIB_HANDLE hrib, RtToken name) {
    ...
    if (rib->ritable && !(rib->status & kRIB_STATUS_BEGUN) && rib->ritable[kRIB_BEGIN]) {
...
int RibEnd(RIB_HANDLE hrib) {
    ...
    if (rib->ritable && !(rib->status & kRIB_STATUS_ENDED) && rib->ritable[kRIB_END]) {
...
int RibOption(RIB_HANDLE hrib, RtToken name, RtToken token, RtPointer param) {
    ...
    if (rib->ritable && rib->ritable[kRIB_OPTION]) {
...
int RibGetChar(RIB_HANDLE hrib) {
    ...
    if (rib) {
        ...
        if ((rib->status & kRIB_STATUS_SAVETOBUFFER) && (pbuf = rib->pbuf) != NULL) {
            *(pbuf->pchar) = c;
            pbuf->buflength++;
...
int RibIgnoreLastChar(RIB_HANDLE hrib) {
    ...
    if (rib && (pbuf = rib->pbuf) != NULL && (rib->status & kRIB_STATUS_SAVETOBUFFER)) {
        /* Check if the rib->pbuf->pchar isn't at the very beginning
         *    of a RIB_BUFFER struct, because decrementing the
         *    pointer could take the pointer into la-la land.
         */
...
int RibClose(RIB_HANDLE hrib) {
    ...
    if (rib && !(rib->status & kRIB_STATUS_SUBFILEOPEN)) {
        ...
        if (!(rib->status & kRIB_STATUS_USINGSHARED)) {
            ...
            p->status |= (rib->status & (kRIB_STATUS_FRAMEBLOCK | kRIB_STATUS_WORLDBLOCK));
...
int RibCloseFile(RIB_HANDLE hrib) {
    ...
    if (rib && !(rib->status & kRIB_STATUS_STDIN) && !(rib->status & kRIB_STATUS_SUBFILEOPEN) && !fclose((FILE *)rib->fp)) {
        rib->fp = NULL;
        if (rib->parent) {
            gRibNSubfiles--;
            if (gRibNOpenSubfiles <= gRibMaxFiles) {
