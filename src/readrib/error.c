...
int RibDefaultErrorHandler(RIB_HANDLE hrib, int code, int severity, PRIB_ERROR error) {
    ...
    if (s) {
        ...
        if (error != NULL && error->type < kRIB_ERRTYPE_LAST && (rib->options & kRIB_OPTION_ERRMSG_VERBOSE)) {
            i = error->type;
            if (i >= kRIB_ERRTYPE_FIRST_MISSING && i <= kRIB_ERRTYPE_LAST_MISSING) {
                ...
                switch (i) {
                    ...
                    case kRIB_ERRTYPE_UNDEFINED_REQUEST:
                        s = "RI request";
                        t = error->unexpected.user_string;
                        break;
