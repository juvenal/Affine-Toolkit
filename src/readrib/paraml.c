...
#define CheckForError(rc) \
    if (rc)               \
    goto Error
#define CheckForErrorAndCleanUp(rc) \
    if (rc)                         \
    goto ErrorCleanUp
...
int RibReadParameterList(RIB_HANDLE hrib, int require, int uniform, int varying, int vertex, RtInt *pn, RtToken **paTokens, RtPointer **paParams) {
    ...
    for (;;) {
         ...
         *    "texturename".
         */
        rc = RibReadString(rib, kRIB_BRACKETS_NONE, &s);
        CheckForError(rc);
